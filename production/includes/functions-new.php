...
function login_user($username, $password, $asset_id)
{
    ...
    $query = "SELECT * FROM staff WHERE access = 1 AND ic = ? ";
    $stm = $connection->prepare($query);
    $stm->bind_param("s", $username);
    $stm->execute();
    $resrow = $stm->get_result();
    ...
    if (($db_user_role == "Admin" || $db_user_role == "Pendaftar" ) && $db_user_access == "1") { // || $db_user_role == "Juruteknik"
        if ($password == password_verify($password, $db_user_password)) { //    password_verify($password, $db_user_password)   $db_user_password
            ...
            $insert_query = "INSERT INTO login_session (staff_id, staff_ic, login_time, last_seen_time) VALUES (?, ?, ?, ?)";
            $sins_query = $connection->prepare($insert_query);
            $sins_query->bind_param("isss",$db_user_id, $db_username, $login_time, $login_time);
