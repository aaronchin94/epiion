...
function Jurutekniklist($technician_id, $connection, $row)
{
    $query_technician = "
    SELECT name, technician_id, jawatan, lokasi FROM juruteknik_view
    ORDER BY name ASC";
    ...
    foreach ($technician_list as $technician) {
        if ($technician_id == $technician['technician_id']) {
            echo '<option value="' . $technician["technician_id"] . '" selected>' . $technician["name"] . '</option>';
        } else {
            echo '<option value="' . $technician["technician_id"] . '">' . $technician["name"] . '</option>';
        }
    }
}
                                            ...
                                            <div class="x_content" style="display: block; text-align: center;">
                                                <div class="table-responsive">
                                                    <table id="datatable-responsive"
                                                        ...
                                                        <tbody>
                                                            <?php
                                                            $query_juruteknik = "SELECT * FROM juruteknik_view order by name";
                                                            ...
                                                            while ($rowk = mysqli_fetch_assoc($result)) {
                                                                $technicianid = $rowk['technician_id'];
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-4 col-sm-6 ">
                                    <select id="technician_id" name="technician_id" class="form-control" required=""
                                        onchange="getvalue()">
                                        <?php
                                        Jurutekniklist($technician_id, $connection, $row);
                                        ?>
                                    </select>
                                </div>
                            </div>
